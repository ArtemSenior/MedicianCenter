...
namespace MedicianCenter.Doctor
{
    public partial class DoctorMainForm : Form
    {
        ...
        {
            this.Text = $"Должность: Врач, ФИО: {StateSingleton.getInstance().authDoc.surname} {StateSingleton.getInstance().authDoc.name} {StateSingleton.getInstance().authDoc.middle_name}";
        ...
        private void UpdateFutureVisitsDataGrid()
        {
            using (Database.Model.Context db = new Context())
            {
                ...
                FutureVisitsDataGridView.Columns["status"].HeaderText = "Статус";
        ...
        private void CreateCertificate(string path, med_card mc, DateTime begin, DateTime end)
        {
            ...
            doc.InsertParagraph($"СПРАВКА О НЕТРУДОСПОСОБНОСТИ").Bold().FontSize(32);
            doc.InsertParagraph().Bold().FontSize(23);
            ...
            doc.InsertParagraph($"\t\t\t(Подпись, дата)").FontSize(9);
            doc.InsertParagraph($"\t\t\tМ.П.").FontSize(9);

            doc.Save();
        }

        private void OpenExtractForm(string path, med_card mc)
        {
            NextVisitForm nvf = new NextVisitForm(path, mc);
            nvf.ShowDialog(this);
        }

        private void OpenRedirectForm(string path, med_card mc)
        {
            RedirectForm rf = new RedirectForm(path, mc);
            rf.ShowDialog(this);
        }
